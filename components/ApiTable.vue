...
<script>
import { getDataAsynC } from '../utils/crud'
export default {
  name: 'ApiTable',
  props: {
    entity: {
  ...
  data() {
    return {
      ...
      initFilters: {},
      initialPage: 1,
      dateColumns: ['columnCreatedAt', 'columnUpdatedAt', 'columnDeadline'],
      customColumns: {
    ...
    options() {
      return {
        ...
        requestFunction: async (params) => {
          ...
          this.options().customFilters.forEach((key) => {
            if (typeof params[key] !== 'undefined' && params[key] !== '') {
              if (this.dateColumns.includes(key)) {
                queryParams[key.slice(6, key.length)] = params[key]
              } else if (key.indexOf('.name') > 0) {
                queryParams[key.replace('.name', '.id')] = params[key]
          ...
          // clear error
          // this.$store.commit(
          //   `${this.entity.toLowerCase()}/${decamelize(
          //     this.entity
          ...
          // const link = 'http://localhost:3001/admin'
          ...
          //   //     count: response.data.page.total_rows,
          //   //   }
          //   //   return data
          //   //   // resolve(data)
          //   // })

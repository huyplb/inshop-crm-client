...
export const actions = {
  async login({ commit }, data) {
    ...
    const link = 'http://localhost:3001/auth/login'
    return await this.$axios
      .post(link, data)
      .then((response) => {
        commit('AUTH_UPDATE_TOKEN', response.data.access_token)
  ...
  async refreshToken({ commit, getters }) {
    const link = 'http://localhost:3001/refresh'
  ...
  async loginByToken({ commit, getters }, token) {
    // const link = process.env.NUXT_ENV_API_URL + '/frontend/login/' + token
    const link = 'http://localhost:3001/token'
    return await this.$axios
      .get(link)
      .then((response) => {
        commit('AUTH_UPDATE_TOKEN', response.data.token)
